...
class DemoTest(object):
    ...
    def test_demo(self, temporary_dict):
        ...
        if type(api) != str:
            api = str(api)
        payload = temporary_dict.get("body")
        # 请求体
        if payload:
            if type(payload) != str:
                payload = str(payload)
        ...
        expected_time = temporary_dict.get("expected_time")
        # 预期的响应时间
        if expected_time:
        ...
        if expected_code == actual_code:
            ...
            if expected_time:
                if actual_time <= expected_time:
                    logger.info("{}>>>实际的响应时间与预期的响应时间断言成功", case_name)
                else:
                    logger.error("{}>>>实际的响应时间大于预期的响应时间！！！", case_name)
